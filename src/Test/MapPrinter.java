...
public class MapPrinter extends JComponent{
	...
	private MapPrinter(int numPoints, int numEdges) {
	...
	public static void main(String[] args) {
		...
		final MapPrinter comp = new MapPrinter(50, 8);
		...
		for (Vertex v : comp.map.getVertices()) {
			Label label = new Label(v.tag + "");
			int len = String.valueOf(v.tag).length();
			label.setBounds((int) (8 * Math.floor(v.getX())) - 6 * len, (int) (8 * Math.floor(v.getY())) - 8, 11 * len, 15);
			comp.add(label);
		}
